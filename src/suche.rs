...
pub fn suche_in_index(s: &str) -> Result<SuchErgebnisse, String> {
    ...
    for (_score, doc_address) in top_docs {
        ...
        let land = retrieved_doc
            .get_first(land)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!("Dokument {doc_address:?}: Fehlendes Feld \"land\""))?;
        let amtsgericht = retrieved_doc
            .get_first(amtsgericht)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!(
                "Dokument {doc_address:?}: Fehlendes Feld \"amtsgericht\""
            ))?;
        let grundbuch_von = retrieved_doc
            .get_first(grundbuch_von)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!(
                "Dokument {doc_address:?}: Fehlendes Feld \"grundbuch_von\""
            ))?;
        let blatt = retrieved_doc
            .get_first(blatt)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!(
                "Dokument {doc_address:?}: Fehlendes Feld \"blatt\""
            ))?;
        let abteilung = retrieved_doc
            .get_first(abteilung)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!(
                "Dokument {doc_address:?}: Fehlendes Feld \"abteilung\""
            ))?;
        let lfd_nr = retrieved_doc
            .get_first(lfd_nr)
            .and_then(|s| s.as_text())
            .map(|s| s.to_string())
            .ok_or(format!(
                "Dokument {doc_address:?}: Fehlendes Feld \"lfd_nr\""
            ))?;
