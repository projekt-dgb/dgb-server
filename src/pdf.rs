use crate::models::{BvEintrag, Grundbuch, Titelblatt};
use hyphenation::{Language, Load, Standard};
use printpdf::{
    BuiltinFont, Cmyk, Color, IndirectFontRef, Line, Mm, PdfDocumentReference, PdfLayerReference,
    Point, Pt,
};
use regex::Regex;
        ...
        for i in 0..zu_ab_len {
            rows.push(PdfTextRow {
                texts: vec![
                    ...
                    eintraege
                        .get(i)
                        .map(|bvz| bvz.get_eigentuemer().text())
                        .unwrap_or_default(),
                    grundlagen_eintragungen
                        .get(i)
                        .map(|bvz| bvz.bv_nr.text())
